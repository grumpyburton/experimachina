...
export class ControlListComponent implements AfterViewInit{
  ...
  openEditControlEligibilityDialog(exp: Control): void {
    const dialogRef =
        this.dialog.open(DialogControlEligibility,{
          width:'80%',
          data: exp
        });
...
// ------------------------------------------------
// DialogControlEligibility Component
// ------------------------------------------------
@Component({
  selector: 'dialog-control-eligibility',
  templateUrl: 'dialog-control-eligibility.html',
  styleUrls: ['./control-list.component.css'],
  standalone: true,
  imports: [MatDialogModule, MatButtonModule, MatInputModule, MatIconModule, MatCardModule,
    FormsModule, MatSlideToggleModule, MatDatepickerModule, MatGridListModule, MatCheckboxModule, MatTableModule],

})
export class DialogControlEligibility {
  ...
  constructor(public dialogRef: MatDialogRef<DialogControl>,
  ...
  saveAndClose()
  {
    ...
    this.apiService.saveControl(this.data).subscribe(
        // do nothing
    );
