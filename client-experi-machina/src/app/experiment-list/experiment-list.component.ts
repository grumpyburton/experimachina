...
export class ExperimentListComponent implements AfterViewInit{
  ...
  openEditExperimentEligibilityDialog(exp: Experiment): void {
    const dialogRef =
        this.dialog.open(DialogExperimentEligibility,{
          width:'80%',
          data: exp
        });
...
// ------------------------------------------------
// DialogExperimentEligibility Component
// ------------------------------------------------
@Component({
  selector: 'dialog-experiment-eligibility',
  templateUrl: 'dialog-experiment-eligibility.html',
  styleUrls: ['./experiment-list.component.css'],
  standalone: true,
  imports: [MatDialogModule, MatButtonModule, MatInputModule, MatIconModule, MatCardModule, FormsModule,
    MatSlideToggleModule, MatDatepickerModule, MatGridListModule, MatCheckboxModule, MatTableModule],

})
export class DialogExperimentEligibility {
  ...
  constructor(public dialogRef: MatDialogRef<DialogExperiment>,
  ...
  saveAndClose()
  {
    ...
    this.apiService.saveExperiment(this.data).subscribe(
        // do nothing
    );
